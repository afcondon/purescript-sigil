...
layoutADT opts =
  let
    ...
    -- Header text width
    headerText = "data " <> opts.name <> if Array.length opts.typeParams > 0
    ...
    dataKw = LText
      { x: 8.0, y: headerH / 2.0, text: "data "
      , fontSize: 12.0, style: "fill:" <> colors.keyword <> ";font-weight:600;" }
    dataKwW = textWidth ctx.charWidth "data "
    nameNode = LText
      { x: 8.0 + dataKwW, y: headerH / 2.0, text: opts.name
      , fontSize: ctx.fontSize, style: "fill:" <> adtColors.headerText <> ";font-weight:700;" }
    paramStartX = 8.0 + dataKwW + textWidth ctx.charWidth opts.name + 6.0
    paramAcc = Array.foldl (\acc p ->
      let pillR = renderSmallPill ctx acc.px 6.0 p (headerH / 2.0)
    ...
    ctorNodes = Array.foldl (\acc idx ->
      case Array.index opts.constructors idx of
        Nothing -> acc
        Just ctor ->
          let
              ...
              case Array.index ctor.args argIdx of
                ...
                Just ast ->
                  let
                    ...
                    ax = a.ax + spacing
                    argR = renderNode ctx ax (cy - ctx.lineHeight / 2.0) ast
